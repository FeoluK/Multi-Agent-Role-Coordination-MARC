"""MAPPO baseline sweep (centralized-critic CTDE; Yu et al. 2022).

The real partial-observability baseline: MPE simple_spread is partially
observable, so MAPPO's centralized critic is a non-trivial control vs
vanilla IPPO. Also run on Overcooked (fully observable -> MAPPO should
~= IPPO; forecloses the "you didn't even try MAPPO" objection).

  - MPE  : mappo x N=3/6/9 x 3 seeds   (tag mappo_mpe_spread_N)
  - OC   : mappo x 5 layouts x 3 seeds (tag mappo_oc)
...
def spec(idx):
    ...
    return (f"TAG={tag} CONFIG={cfg} KIND=mappo SEED={seed} "
